...
- Check the data provenance: for a self-sourced run, confirm validation/data_check.json verdict is PASS and data/SOURCE.md documents a real, reproducible source.
- Check the literature grounding: positioning/prior-knowledge claims must cite paper/references.bib; spot-check that references.bib entries are real, verifiable works (no fabricated authors, DOIs, or venues) and that novelty is not overstated relative to them.
